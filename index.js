...
function unSurround(string) {
    return string.trim().substring(1, string.trim().length-1).replace(twoDoubleQuoteRegex, '"')
}

/**
 * Returns an array of objects by parsing a CSV string.
 * @param {string} csv Source of the object as a string containing CSV data 
 * @param {object=} [options={seperator:",", isSurrounded: true}] Specifies the properties of the output
 * @param {string} options.seperator Seperator that is used in the CSV to seperate the values
 * @param {Boolean} options.isSurrounded Specifies if the values are seperated by double quotes, if true, assumes that all the values are surrounded by double quotes
 * @returns {object[]} Object containing parsed CSV data from the passed CSV string
 */
exports.parse = function (csv, options) {
    ...
    const lines = csv.split(newLineRegex).filter(line => line.length>1).map(line => line.trim());
    const firstLine = lines.shift();
    lines.pop();
    const keys = firstLine.split(seperator)
        .map(key => (isSurrounded == false) ? key.trim() : unSurround(key))
    const parsedCsvArray = []
    for (let index = 0; index < lines.length; index++) {
        const line = lines[index];
        const entries = line.split(seperator).map(entry => (isSurrounded == false) ? entry.trim() : unSurround(entry));
 ...
 * @param {object[]} objects Source of the CSV string as an array of objects  
 * @param {object=} [options={seperator:",", isSurrounded: true}] Specifies the properties of the output
 ...
 * @returns {string} A string containing stringified object data from the passed array of objects. 
 */
